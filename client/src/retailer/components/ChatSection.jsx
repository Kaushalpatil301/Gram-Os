...
export default function ChatSection({ activeChats }) {
  ...
  const [messages, setMessages] = useState({
    // Pre-populate with some initial greetings from a farmer
    "Ram Singh": [
      { sender: "Farmer", text: "Hello! Yes, the organic tomatoes are ready for dispatch.", time: "10:05 AM" }
    ]
  });

  const handleSend = () => {
    if (!message.trim() || !selectedChat) return;
    const newMsg = { sender: "Retailer", text: message, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
  ...
  if (!activeChats || activeChats.length === 0) {
    return (
      <div className="bg-white rounded-3xl shadow-xl border border-green-100 p-12 text-center mt-6">
        ...
        <p className="text-gray-500">Farmers who accept your connection requests will appear here.</p>
      </div>
    );
  }
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-emerald-600">Online • Farmer</p>
              </div>
            </div>
            <button 
            ...
            {!messages[selectedChat.name]?.length && (
              <div className="text-center mt-10">
                <div className="inline-block bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
                  <p className="text-gray-500 text-sm font-medium mt-1">Chat is ready. Send a message!</p>
                </div>
              </div>
            )}
            {currentMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.sender === "Retailer" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[75%] rounded-2xl px-4 py-3 shadow-sm ${msg.sender === "Retailer" ? "bg-emerald-600 text-white rounded-tr-sm" : "bg-white border border-gray-100 text-gray-800 rounded-tl-sm"}`}>
                  <p className="text-sm font-medium leading-relaxed">{msg.text}</p>
                  <p className={`text-[10px] mt-1.5 text-right font-medium ${msg.sender === "Retailer" ? "text-emerald-100" : "text-gray-400"}`}>{msg.time}</p>
                </div>
              </div>
            ))}
          </div>
