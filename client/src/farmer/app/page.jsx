...
import {
  ShoppingCart, Users,
  Award, Landmark, Warehouse, QrCode, Map,
  LogOut, User, Menu, Sprout, Bell, Package, MessageSquare, DollarSign
} from "lucide-react";

const NAV_ITEMS = [
  { id: "marketplace", label: "Input Market", icon: ShoppingCart, color: "text-emerald-600" },
  { id: "produce", label: "My Produce", icon: Warehouse, color: "text-emerald-600" },
  { id: "workforce", label: "Workforce", icon: Users, color: "text-orange-600" },
  ...
  { id: "loans", label: "Loan Requests", icon: DollarSign, color: "text-blue-600" },
...
export default function FarmerPage({ onLogout }) {
  ...
  const handleAcceptConnect = (notif) => {
    if (!activeChats.find(chat => chat.id === notif.id)) {
      setActiveChats(prev => [...prev, { id: notif.id, name: notif.retailerName, topic: notif.topic }]);
    }
